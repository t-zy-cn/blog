...
`);const M=C(m)?m:T(m)?g=>m.replace(/:key/g,o(n)).replace(/:name/g,o(g)):()=>null,c={},O=[];for(const g in t){if(b){const r=`${g}${D(b.replace(/:key/g,o(n)))}`;O.push({path:r,frontmatter:{...I(g),blog:{type:"category",key:n},layout:E}}),c[g]={path:L(r),map:{}}}else c[g]={path:"",map:{}};const{map:a}=c[g],f={};for(const r of t[g]){const i=d(r);for(const s of i){if(!(s in a)){const $=M(s);if($){const N=`${g}${D($)}`;O.push({path:N,frontmatter:{..._(s,g),blog:{type:"category",name:s,key:n},layout:k}}),a[s]={path:L(N),indexes:[]}}else a[s]={path:"",indexes:[]};f[s]=[]}f[s].push(r)}}for(const r in f)a[r].indexes=e.addItems(f[r].sort(v).map(({path:i})=>i));if(u){let r=`${n} category in locale ${g}:
`;for(const i in a){const{path:s,indexes:$}=a[i];r+=`${i}: found ${$.length} items${s?` in path: ${s}`:""}
...
${t.env.isDev?pe:""}
`)},re=t=>e=>{e.env.isDebug&&l.info("Options:",t);const{getInfo:p=()=>({}),filter:o=a=>!!a.filePathRelative&&!a.frontmatter.home,metaScope:u="_blog",excerpt:w=!0,excerptSeparator:n="<!-- more -->",excerptLength:d=300,excerptFilter:v=o,isCustomElement:b=()=>!1,category:E=[],type:I=[],slugify:m=a=>a.replace(/[ _]/g,"-").replace(/[:?*|\\/<>]/g,"").toLowerCase()}=t,k=ee(E),_=ne(I),M=new ae;let c=[],O={},g={};return{name:j,define:()=>({__BLOG_META_SCOPE__:u}),extendsBundlerOptions:a=>{q(a,e,"@vuepress/helper")},extendsPage:a=>{w&&v(a)&&!a.data.excerpt&&(a.data.excerpt=W(e,a,{isCustomElement:b,separator:n,length:d})),o(a)&&(a.routeMeta={...u===""?p(a):{[u]:p(a)},...a.routeMeta})},onInitialized:async()=>{const a=V(e,o),f=H(a,M,k,m,e.env.isDebug),r=F(a,M,_,m,e.env.isDebug);await Promise.all([...f.pageOptions,...r.pageOptions].map(async i=>{if(e.pages.findIndex(s=>s.path===i.path)!==-1){l.warn("Overriding existing page:",i.path);const s=e.pages.findIndex($=>$.path===i.path);e.pages.splice(s,1,await U(e,i))}e.pages.push(await U(e,i))})),c=[...f.pageOptions,...r.pageOptions].map(i=>i.path),O=f.categoriesMap,g=r.typesMap},onPrepared:async()=>{await oe(e,M),await B(e,O),await A(e,g),e.env.isDebug&&l.info("temp file generated")},onWatched:(a,f)=>{if("hotReload"in t?t.hotReload:e.env.isDebug){const r=K("pages/**/*.js",{cwd:e.dir.temp(),ignoreInitial:!0}),i=async()=>{const s=V(e,o),$=H(s,M,k,m,e.env.isDebug),N=F(s,M,_,m,e.env.isDebug),P=[...$.pageOptions,...N.pageOptions];await B(e,$.categoriesMap),await A(e,N.typesMap);const R=P.filter(y=>!c.includes(y.path)),S=c.filter(y=>P.every(x=>x.path!==y));R.length&&(e.env.isDebug&&l.info(`Adding new pages: ${R.map(({path:y})=>y).join(", ")}`),await Promise.all(R.map(async y=>{const x=await U(e,y);await Q(e,x),await X(e,x),e.pages.push(x)}))),S.length&&(e.env.isDebug&&l.info(`Removing following pages: ${S.join(", ")}`),S.forEach(y=>{e.pages.splice(e.pages.findIndex(({path:x})=>x===y),1)})),(S.length||R.length)&&await Y(e),c=P.map(y=>y.path),e.env.isDebug&&l.info("temp file updated")};r.on("add",()=>{i()}),r.on("change",()=>{i()}),r.on("unlink",()=>{i()}),f.push(r)}}}};export{re as blogPlugin};
